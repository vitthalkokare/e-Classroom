...
import useMountedState from "@/app/Components/common/UseMounted";
...
const Countdown = () => {
  ...
  useEffect(() => {
    setMout(true);
    const timeIntervals = allotSubject?.map((v: ClassesProps) => {
    ...
    const lecTime = allotSubject?.map((v: ClassesProps) => {
  ...
  const filteredArray = allotSubject?.filter((item, index) => {
...
const startIN = allotSubject?.filter((item,index)=>(countEndTime[index] !== 0));
